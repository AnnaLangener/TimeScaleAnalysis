...
### Load noisy datasets
Wifi <- read.csv("/Users/annalangener/TimeScaleAnalysis/Datasets_Noise/Wifi_sample.csv")[-1]
App <- read.csv("/Users/annalangener/TimeScaleAnalysis/Datasets_Noise/App_sample.csv")[-1]
ESM <- read.csv("/Users/annalangener/TimeScaleAnalysis/Datasets_Noise/ESM_time_sample.csv")[-1]
```
...
```{r echo=TRUE}
Wifi$recorded_naive <- as.POSIXct(Wifi$recorded_naive) 
App$recorded_naive <- as.POSIXct(App$recorded_naive)
...
for(i in 1:nrow(App)){
   App$end_time[i]  <- App$recorded_naive[i] + App$duration[i] #Add start time (recorded_naive) and duration together to calculate the end time
}
```
...
App_persecond <- App %>%
  transmute(genre, Date = vec_seq(recorded_naive, end_time, 1)) %>% #transmute() will create a new variable (Date) based on the vec_seq() function (which takes the following arguments: from = recorded_naive, to = end_time, by = 1 (by one second))
...
for (i in 1:nrow(ESM)) {
  ...
  TOTAL_MACHASHES_number <-
    Wifi[(Wifi$recorded_naive > (time - rule_int * 60 * 60)) &
           (Wifi$recorded_naive < time), ] %>% nrow() # Here we calculate the total number of recorded wifi connections (X hours before but not after the ESM was filled out)
  
  UNIQUE_MACHASHES_number <-
    Wifi[(Wifi$recorded_naive > (time - rule_int * 60 * 60)) &
           (Wifi$recorded_naive < time), ] %>% .[!duplicated(.$mac_hash),]  %>% nrow() # Here we calculate the number of unique recorded wifi connections (X hours before but not after the ESM was filled out)
...
##### Violin Plots
To better understand how the data varies over different time scales, it is a good start to visualize it. For this, we are using violin plots. Violin plots show the distribution of the data, which is here aggregated three hours before ESM was filled out. In our paper, we explore other time scales as well. The code provided is also able to display more than one time scale if those are present in the dataset. 
...
# We use ggplot to create the violin plots
...
We use the caret package for our analysis. More information about the caret package and time series cross-validation can be found here:
https://topepo.github.io/caret/data-splitting.html#data-splitting-for-time-series

```{r echo=TRUE, warning=FALSE}
# First we omit rows with missing data. How to handle missing data (once it is identified), for example, whether to impute missing data or whether to exclude missing data, is another important decision. For simplicity we decided to exlcude missing data here. The participant used in our example in the paper did not have any missing data.
...
for(k in c(min_window:max_window)){ # here we loop through the different moving window sizes
  ...
  fitControl <- trainControl(method = "LOOCV",
                             number = 1) # to use leave-one-out-cross-validation to choose the best hyperparameters (= number of predictors)  in the training set.
  ...
  for(i in 1:length(trainSlices)){
    ...
    model<- train(formula2, data=FeatureOverview[trainSlices[[i]],],trControl = fitControl, method="rf", na.action = na.omit) #Here the prediction model is build
...
As a next step, we calculate the prediction accuracy and plot the results. The code provided can plot the results for different moving window sizes. In this example, the prediction accuracy is very low (even negative), since we used a noisy dataset and only a few predictors.
